...
- **La parrilla completa (4.º producto, "Lights Out Legends Live"):** cuadro panorámico con los 12 autos, en su propia sección debajo del inicio (`src/three/GridShowcase.jsx`). Muestra solo el cuadro, como es en la realidad: el cuadro completo con las 12 bases, el marco negro profundo y la tira LED blanca cálida escondida en el borde interior, que ilumina las paredes internas del marco y el fondo pegado al borde. Cada auto sobresale en 3D con su silueta. Con el cursor, la vista se corre apenas de lado para que se note el relieve. Al señalar o tocar un auto aparece su equipo (lista en `cars` de `src/lib/models.js`, de izquierda a derecha: Ferrari, McLaren, Red Bull, Mercedes, Aston Martin, Alpine, Haas, Racing Bulls, Williams, Kick Sauber, APXGP y Audi). En el celular se recorre deslizando de lado. En Shopify se reconoce con la etiqueta `modelo:parrilla` o si el título incluye "Lights Out" / "Legends".
