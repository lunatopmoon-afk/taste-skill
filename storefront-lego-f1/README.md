...
- **La parrilla completa (4.º producto, "Lights Out Legends Live"):** cuadro panorámico con 12 autos, en su propia sección debajo del inicio (`src/three/GridShowcase.jsx`). Al aparecer en pantalla corre la largada de F1: se encienden 5 luces rojas una por una, se apagan de golpe y se prende el neón blanco del marco. Cada auto con su base sobresale en 3D. Al señalar o tocar un auto se resalta y aparece su equipo (lista en `cars` de `src/lib/models.js`). En el celular se recorre deslizando de lado. En Shopify se reconoce con la etiqueta `modelo:parrilla` o si el título incluye "Lights Out" / "Legends".
